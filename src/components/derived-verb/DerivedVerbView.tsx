...
import { Database, Repeat } from "lucide-react";
import { BLANK_PLACEHOLDER } from "@/lib/derivedVerbTypes";
import DerivedVerbFeedback from "./DerivedVerbFeedback";

const TOTAL_EXERCISES = 5;

export default function DerivedVerbView() {
  const { currentExercise, answers, submitDerivedVerb, nextDerivedVerb } = useDerivedVerbStore();
  ...
  const handleSubmit = () => {
    ...
    setIsSubmitted(true);
    submitDerivedVerb([userInput1.trim(), userInput2.trim()]);
  };
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 md:py-10 flex flex-col animate-slide-up">
      <div className="flex items-center justify-between mb-4">
        <div className="text-xs font-semibold text-zinc-400">
          Exercício <span className="text-emerald-400">{currentNumber}</span> de {TOTAL_EXERCISES}
        </div>
      ...
      <div className="border border-dark-border bg-dark-card rounded-2xl p-6 shadow-xl mb-6">
        ...
        <p className="text-base sm:text-lg text-zinc-200 leading-relaxed mb-6 p-4 rounded-xl bg-zinc-900/60 border border-dark-border">
          {before}
          <span className="inline-block min-w-[80px] border-b-2 border-emerald-500 text-transparent select-none">
            {BLANK_PLACEHOLDER}
          </span>
          {middle}
          <span className="inline-block min-w-[80px] border-b-2 border-emerald-500 text-transparent select-none">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1.5">
                  1ª lacuna — {currentExercise.rootVerb.infinitive} ({currentExercise.rootVerb.tense})
                </p>
                <input
                  ref={inputRef1}
                  type="text"
                  ...
                  className="w-full px-4 py-4 rounded-xl border border-dark-border bg-zinc-900/60 text-zinc-100 font-mono text-lg font-bold placeholder:text-zinc-600 placeholder:font-normal outline-none focus:border-emerald-500/60 focus:ring-1 focus:ring-emerald-500/30 transition-all"
                  autoComplete="off"
                  autoCorrect="off"
                  autoCapitalize="off"
                  spellCheck={false}
              ...
              <div>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1.5">
                  2ª lacuna — {currentExercise.derivedVerb.infinitive} ({currentExercise.derivedVerb.tense})
                </p>
                <input
                  ref={inputRef2}
                  type="text"
